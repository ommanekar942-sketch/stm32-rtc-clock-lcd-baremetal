...
typedef struct
{
	...
	uint32_t TxLen;
	uint32_t RxLen;
	uint8_t TxRxState;     // --> I2C application state
	uint8_t DevAddr;       // To store slave/device addresses
	uint8_t Sr;
	uint32_t RxSize;
}I2C_Handle_t;
...
// I2C event and error application macros
...
void I2C_SlaveEnableDisableCallbackEvents(I2C_REGDEF_t *pI2Cx, uint8_t EnorDi);		// --> use when STM32 work as the slave and arduino as master
